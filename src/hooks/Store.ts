 ...
 * }
 *
 * const s = StoreFactory.create<ISomeContainerStore>({ isLogin: false }, true)
...
export class StoreFactory {
  static create<T extends PlainObject> (store: Partial<T>, strict: boolean = false) {
    return new Store<T>(store, strict)
  }
}

class Store<T extends PlainObject> {
  /**
    ...
    useEffect(() => {
      ...
      const listen = () => {
        ...
        for (let key in cache) {
          ...
          if (Object.is(cache[key], (updatedMappedStore as any)[key])) continue
